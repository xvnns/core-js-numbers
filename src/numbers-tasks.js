...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const magnitude1 = (x1 ** 2 + y1 ** 2) ** (1 / 2);
  const magnitude2 = (x2 ** 2 + y2 ** 2) ** (1 / 2);
...
function getFibonacciNumber(index) {
  ...
  let i = 0;
  while (i < index) {
    newNumber = nextNumber + currentNumber;
...
function getCountOfOddNumbers(number) {
  return roundToSmallestInteger((Math.abs(number) + 1) / 2);
}
